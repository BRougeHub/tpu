...
def detection_loss(cls_outputs, box_outputs, map_output, labels, params):
  ...
  Args:
    ...
    box_outputs: an OrderDict with keys representing levels and values
      ...
      num_anchors * 4].
    map_output: a tensor of size [batch_size, height, width, num_classes]
  ...
  for level in levels:
    ...
    box_losses.append(
        _box_loss(
            ...
            num_positives_sum,
            delta=params['delta']))
    map_loss = _segmentation_loss(map_output, labels['mask'],
                                  params)
...
def _model_fn(features, labels, mode, params, model, variable_filter_fn=None):
  ...
  if mode == tf.estimator.ModeKeys.PREDICT:
    ...
    for level in levels:
      ...
      predictions['map_output'] = map_output
  ...
  total_loss, cls_loss, box_loss, map_loss = detection_loss(cls_outputs, 
                                                  box_outputs, map_output,
                                                  labels, params)
